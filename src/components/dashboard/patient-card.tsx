...
export function PatientCard({
  ...
  return (
    <Link href={`/patients/${doctorPatientId}`} className="block group">
      ...
      >
        <div className="absolute left-0 top-2 bottom-2 w-0.75 rounded-r-full bg-[#7c3aed] opacity-0 group-hover:opacity-100 transition-opacity" />

        {/* Avatar */}
        <div className="shrink-0" style={{ paddingBottom: "2px", background: "linear-gradient(180deg, #d4cbe6, #c8bedd)", borderRadius: "9999px" }}>
          <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "linear-gradient(135deg, #e8e2f6, #ddd6ee)" }}>
            <span className="text-[0.78rem] text-[#7c3aed]" style={{ fontWeight: 600 }}>{initials}</span>
          </div>
        </div>

        {/* Name */}
        <div className="flex-1 min-w-0" style={{ minWidth: "180px" }}>
          <span className="text-[0.95rem] block truncate" style={{ fontWeight: 600, color: "#2d2b3d" }}>{name}</span>
          <p className="text-[0.82rem] mt-0.5" style={{ fontWeight: 400, color: "#8e8aa0" }}>
        ...
        <div className="shrink-0 hidden sm:flex items-center" style={{ minWidth: "110px" }}>
          <span
            className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full text-[0.8rem]"
            style={{ fontWeight: 600, color: status.color, background: status.bg, boxShadow: "0 2px 0 rgba(0,0,0,0.03), 0 1px 4px rgba(0,0,0,0.02)" }}
          >
        ...
        <div className="shrink-0 hidden sm:block" style={{ minWidth: "60px" }}>
          <span className="text-[0.95rem]" style={{ fontWeight: 700, color: status.color }}>{complianceOverall}%</span>
        </div>

        {/* Condition */}
        <div className="shrink-0 hidden md:block" style={{ minWidth: "90px" }}>
          <span className="text-[0.9rem]" style={{ fontWeight: 400, color: "#5a5773" }}>{conditionLabel}</span>
        </div>

        {/* Last Check-In */}
        <div className="shrink-0 hidden lg:block" style={{ minWidth: "110px" }}>
          <span className="text-[0.9rem]" style={{ fontWeight: 400, color: "#8e8aa0" }}>{lastCheckInText}</span>
        </div>
